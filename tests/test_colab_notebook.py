...
def test_all_colab_phases_carry_the_saved_runtime_fingerprint(tmp_path, monkeypatch):
    ...
    assert namespace["EXPERIMENT_VERSION"] == "summary-v7"
    assert namespace["REPO"] == Path("/content/agent-monitor-context-audit-summary-v7")
    ...
    for phase in ("pilot", "development", "test"):
        ...
        assert config.structured_summary_mode == "schema_citations_separate_ids_v1"
        ...
        saved = json.loads((configuration / f"{phase}-summary-v7.json").read_text())
...
def test_builder_defaults_start_an_inert_v7_pilot(tmp_path, monkeypatch, capsys):
    import runpy
